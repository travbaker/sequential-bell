...
Maximize the tradeoff between S*(A2,B2) and S(A1,B1) when A1,B1 restricted to measure unbiased observables.
4 fewer parameter in the general case... 4 strengths, 8 angles plus one parameter for the two-qubit state.
...
def CHSH_first_eval(x):
    ...
    rho_initial = generic_entangled(x[12])
...
# =============================================================================
# Objective. Eq 10 in the preprint
# =============================================================================
...
bnds=[(0,1),(0,np.pi),(0,2*np.pi)]*4 + [(0,np.pi/2)]
...
def uniform_init_sampling(num_population_members):
    ...
    first_member = np.array([1,0,0,1,np.pi/2,0,1,np.pi/4,0,1,-np.pi/4,0,np.pi/4])
...
if root.success==True and root_polished.success==True:
    ...
    outputs = open('unbiased_converged_data_with_state.txt', 'a') 
